...
const DEFINICIONES = {
  ...
  palabras_pago_excluido: { tipo: 'lista' },
  ...
  venta_oficina_habilitada: { tipo: 'booleano' },
  credito_requiere_titular: { tipo: 'booleano' },
  cierre_padron: { tipo: 'fecha_hora' },
  zona_horaria: { tipo: 'zona_horaria' },
  numero_permiso: { tipo: 'texto_opcional' },
  modo_exhibicion: { tipo: 'booleano' },
  vigencia_promocion: { tipo: 'texto' },
  fecha_sorteo: { tipo: 'texto' },
  telefono_aclaraciones: { tipo: 'texto_opcional' },
  whatsapp_link: { tipo: 'url_opcional' },
  intervalo_correo_minutos: { tipo: 'numero' },
  hora_limite_archivo: { tipo: 'hora' },
};
...
function validarValor(clave, valorCrudo) {
  ...
  // texto_opcional: puede quedar vacío (p. ej. numero_permiso mientras la
  // SEGOB no lo otorgue).
  if (definicion.tipo === 'texto_opcional') return { ok: true, valor };
  // url_opcional: vacío permitido; con valor, debe ser una liga https.
  if (definicion.tipo === 'url_opcional') {
    if (!valor) return { ok: true, valor };
    if (!/^https:\/\/\S+$/.test(valor)) {
      return { ok: false, error: 'Debe ser una liga https:// (ej. https://wa.me/5216251234567).' };
    }
  ...
  switch (definicion.tipo) {
    ...
    case 'hora':
      if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(valor)) {
        return { ok: false, error: 'Formato esperado: HH:MM (24 horas).' };
      }
      return { ok: true, valor };
    case 'formato_boleto':
